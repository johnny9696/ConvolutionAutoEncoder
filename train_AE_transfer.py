...
def train_and_eval(rank,n_gpu, hps):
    ...
    #load model dict
    checkpoint_path = '/media/data_drive/autoencoder/log/kernel5'
    utils.load_checkpoint(checkpoint_path,AE_model)

    CAC_model = model (encoder_dim=hps.model.encoder_dim, hidden_1dim=hps.model.hidden_dim1,
    hidden_2dim=hps.model.hidden_dim2, kernel=hps.model.kernel, hps = hps).to(device)
    ...
    if hps.n_gpus>1:
        ...
        AE_model=DistributedDataParallel(CAC_model,device_ids=[rank]).to(device)
